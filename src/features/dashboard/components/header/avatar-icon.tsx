...
interface AvatarIconProps {
  initialIcon: 'notification' | 'settings' | 'chat'
  currentState: IconState
  onStateChange: (icon: string, newState: IconState) => void
}

export function AvatarIcon({ initialIcon, currentState, onStateChange }: AvatarIconProps) {
  const router = useRouter()

  const handleClick = () => {
    const newState = {
      bgColor: currentState.bgColor === '#F0F2F5' ? '#3182CE' : '#F0F2F5',
      color: currentState.color === '#000000' ? '#FFFFFF' : '#000000',
    }
    onStateChange(initialIcon, newState)
  }
