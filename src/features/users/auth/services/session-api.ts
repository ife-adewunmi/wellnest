...
export class SessionApi implements SessionApiRequests {
  ...
  public async validateSession(): Promise<SessionResponse> {
    return await request.get(Endpoints.API.concat('/auth/session'))
  }
  ...
  public async extendSession(): Promise<SessionExtendResponse> {
    return await request.post(Endpoints.API.concat('/auth/session/extend'))
  }
  ...
  public async invalidateSession(sessionId?: string): Promise<{ success: boolean }> {
    const endpoint = sessionId 
      ? Endpoints.API.concat(`/auth/session/${sessionId}/invalidate`)
      : Endpoints.API.concat('/auth/session/invalidate')
  ...
  public async invalidateAllSessions(): Promise<{ success: boolean }> {
    return await request.post(Endpoints.API.concat('/auth/session/invalidate-all'))
  }
  ...
  public async getActiveSessions(): Promise<ActiveSessionsResponse> {
    return await request.get(Endpoints.API.concat('/auth/session/active'))
  }
  ...
  public async refreshActivity(): Promise<{ success: boolean }> {
    return await request.post(Endpoints.API.concat('/auth/session/refresh'))
  }
}
