define(['exports'], function (t) {
  ...
  function N(t) {
    return t !== IDBDatabase.prototype.transaction || 'objectStoreNames' in IDBTransaction.prototype
      ? (
        ...
        ? function (...e) {
            return (t.apply(T(this), e), O(x.get(this)))
          }
        : function (...e) {
            return O(t.apply(T(this), e))
          }
      : function (e, ...s) {
          const n = t.call(T(this), e, ...s)
          return (I.set(n, e.sort ? e.sort() : [e]), O(n))
        }
  }
  function C(t) {
  ...
  function O(t) {
    ...
    const e = C(t)
    return (e !== t && (k.set(t, e), B.set(e, t)), e)
  }
  const T = (t) => B.get(t)
  function P(t, e, { blocked: s, upgrade: n, blocking: i, terminated: r } = {}) {
    const a = indexedDB.open(t, e),
      c = O(a)
    return (
      n &&
        a.addEventListener('upgradeneeded', (t) => {
          n(O(a.result), t.oldVersion, t.newVersion, O(a.transaction), t)
        }),
  ...
  function W(t, e) {
    ...
    const s = e.replace(/FromIndex$/, ''),
      n = e !== s,
      i = M.includes(s)
    if (!(s in (n ? IDBIndex : IDBObjectStore).prototype) || (!i && !j.includes(s))) return
  ...
  L = ((t) =>
    v({}, t, {
      get: (e, s, n) => W(e, s) || t.get(e, s, n),
      has: (e, s) => !!W(e, s) || t.has(e, s),
    }))(L)
  try {
  ...
  class F {
    ...
    async getDb() {
      return (
        this.U || (this.U = await P('workbox-expiration', 1, { upgrade: this.B.bind(this) })),
        this.U
      )
    }
  }
  class $ {
    ...
    async expireEntries() {
      if (this.N) return void (this.C = !0)
      this.N = !0
      const t = this.T ? Date.now() - 1e3 * this.T : 0,
        e = await this.j.expireEntries(t, this.O),
        s = await self.caches.open(this.I)
      for (const t of e) await s.delete(t, this.P)
      ;((this.N = !1), this.C && ((this.C = !1), b(this.expireEntries())))
    }
    async updateTimestamp(t) {
      await this.j.setTimestamp(t, Date.now())
    }
    async isURLExpired(t) {
      if (this.T) {
        const e = await this.j.getTimestamp(t),
    ...
    async delete() {
      ;((this.C = !1), await this.j.expireEntries(1 / 0))
    }
  }
  try {
  ...
  class H {
    constructor(t = {}) {
      ;((this.M = t.statuses), (this.S = t.headers))
    }
    isResponseCacheable(t) {
      let e = !0
      return (
        this.M && (e = this.M.includes(t.status)),
        this.S && e && (e = Object.keys(this.S).some((e) => t.headers.get(e) === this.S[e])),
        e
      )
    }
  }
  ...
  class z {
    ...
    async addEntry(t) {
      const e = (await this.getDb()).transaction(Q, 'readwrite', { durability: 'relaxed' })
      ;(await e.store.add(t), await e.done)
    }
    async getFirstEntryId() {
      const t = await this.getDb(),
        e = await t.transaction(Q).store.openCursor()
      return null == e ? void 0 : e.value.id
    }
    async getAllEntriesByQueueName(t) {
      const e = await this.getDb(),
        s = await e.getAllFromIndex(Q, G, IDBKeyRange.only(t))
      return s || new Array()
    }
    async getEntryCountByQueueName(t) {
      return (await this.getDb()).countFromIndex(Q, G, IDBKeyRange.only(t))
    }
    async deleteEntry(t) {
      const e = await this.getDb()
      await e.delete(Q, t)
    ...
    async getEndEntryFromIndex(t, e) {
      const s = await this.getDb(),
        n = await s.transaction(Q).store.index(G).openCursor(t, e)
      return null == n ? void 0 : n.value
    }
    async getDb() {
      return (
        this.U || (this.U = await P('workbox-background-sync', 3, { upgrade: this.k })),
        this.U
      )
    }
    k(t, e) {
      e > 0 && e < 3 && t.objectStoreNames.contains(Q) && t.deleteObjectStore(Q)
      t.createObjectStore(Q, { autoIncrement: !0, keyPath: 'id' }).createIndex(G, G, { unique: !1 })
    }
  }
  class V {
    constructor(t) {
      ;((this.W = t), (this.K = new z()))
    }
    async pushEntry(t) {
      ;(delete t.id, (t.queueName = this.W), await this.K.addEntry(t))
    }
    async unshiftEntry(t) {
      const e = await this.K.getFirstEntryId()
      ;(e ? (t.id = e - 1) : delete t.id, (t.queueName = this.W), await this.K.addEntry(t))
    }
    async popEntry() {
      return this.A(await this.K.getLastEntryByQueueName(this.W))
    }
    async shiftEntry() {
      return this.A(await this.K.getFirstEntryByQueueName(this.W))
    }
    async getAll() {
      return await this.K.getAllEntriesByQueueName(this.W)
    }
    async size() {
      return await this.K.getEntryCountByQueueName(this.W)
    }
    async deleteEntry(t) {
      await this.K.deleteEntry(t)
    }
    async A(t) {
      return (t && (await this.deleteEntry(t.id)), t)
    }
  }
  const J = [
    'method',
  ...
  class X {
    static async fromRequest(t) {
      ...
      for (const s of J) void 0 !== t[s] && (e[s] = t[s])
      return new X(e)
    }
    constructor(t) {
      ;('navigate' === t.mode && (t.mode = 'same-origin'), (this.F = t))
    }
    toObject() {
      const t = Object.assign({}, this.F)
      return (
        (t.headers = Object.assign({}, this.F.headers)),
        t.body && (t.body = t.body.slice(0)),
        t
      )
    }
    toRequest() {
      return new Request(this.F.url, this.F)
    }
    clone() {
      return new X(this.toObject())
    }
  }
  const Y = 'workbox-background-sync',
    Z = new Set(),
    tt = (t) => {
      const e = { request: new X(t.requestData).toRequest(), timestamp: t.timestamp }
      return (t.metadata && (e.metadata = t.metadata), e)
    }
  class et {
    constructor(t, { forceSyncFallback: e, onSync: n, maxRetentionTime: i } = {}) {
      if (((this.$ = !1), (this.H = !1), Z.has(t))) throw new s('duplicate-queue-name', { name: t })
      ;(Z.add(t),
        (this.G = t),
        (this.V = n || this.replayRequests),
        (this.J = i || 10080),
        (this.X = Boolean(e)),
        (this.Y = new V(this.G)),
        this.Z())
    ...
    async pushRequest(t) {
      await this.tt(t, 'push')
    }
    async unshiftRequest(t) {
      await this.tt(t, 'unshift')
    }
    async popRequest() {
      return this.et('pop')
    }
    ...
    async getAll() {
      const t = await this.Y.getAll(),
        e = Date.now(),
        s = []
      for (const n of t) {
        const t = 60 * this.J * 1e3
        e - n.timestamp > t ? await this.Y.deleteEntry(n.id) : s.push(tt(n))
      }
      return s
    }
    async size() {
      return await this.Y.size()
    }
    async tt({ request: t, metadata: e, timestamp: s = Date.now() }, n) {
      const i = { requestData: (await X.fromRequest(t.clone())).toObject(), timestamp: s }
      switch ((e && (i.metadata = e), n)) {
        case 'push':
          await this.Y.pushEntry(i)
          break
        case 'unshift':
          await this.Y.unshiftEntry(i)
      }
      this.$ ? (this.H = !0) : await this.registerSync()
    }
    async et(t) {
      const e = Date.now()
      let s
      switch (t) {
        case 'pop':
          s = await this.Y.popEntry()
          break
        case 'shift':
          s = await this.Y.shiftEntry()
      }
      if (s) {
        const n = 60 * this.J * 1e3
        return e - s.timestamp > n ? this.et(t) : tt(s)
      }
    }
    async replayRequests() {
      ...
      for (; (t = await this.shiftRequest()); )
        try {
          ...
          throw (await this.unshiftRequest(t), new s('queue-replay-failed', { name: this.G }))
        }
    }
    async registerSync() {
      if ('sync' in self.registration && !this.X)
        try {
          await self.registration.sync.register(`${Y}:${this.G}`)
        } catch (t) {}
    }
    Z() {
      'sync' in self.registration && !this.X
        ? self.addEventListener('sync', (t) => {
            if (t.tag === `${Y}:${this.G}`) {
              const e = async () => {
                ...
                this.$ = !0
                try {
                  await this.V({ queue: this })
                } catch (t) {
                  if (t instanceof Error) throw ((e = t), e)
                } finally {
                  ;(!this.H || (e && !t.lastChance) || (await this.registerSync()),
                    (this.$ = !1),
                    (this.H = !1))
                }
              }
  ...
  class rt {
    constructor({ precacheController: t }) {
      ;((this.cacheKeyWillBeUsed = async ({ request: t, params: e }) => {
        const s = (null == e ? void 0 : e.cacheKey) || this.nt.getCacheKeyForURL(t.url)
  ...
  class ht extends q {
    constructor(t = {}) {
      ;((t.cacheName = w(t.cacheName)),
        super(t),
        (this.it = !1 !== t.fallbackToNetwork),
        this.plugins.push(ht.copyRedirectedCacheableResponsesPlugin))
    }
    async _(t, e) {
      const s = await e.cacheMatch(t)
      return (
        s || (e.event && 'install' === e.event.type ? await this.rt(t, e) : await this.ct(t, e))
      )
    }
    async ct(t, e) {
      let n
      const i = e.params || {}
      if (!this.it) throw new s('missing-precache-entry', { cacheName: this.cacheName, url: t.url })
      {
        const s = i.integrity,
          r = t.integrity,
          a = !r || r === s
        ;((n = await e.fetch(
          ...
          s && a && 'no-cors' !== t.mode && (this.ot(), await e.cachePut(t, n.clone())))
      }
      return n
    }
    async rt(t, e) {
    ...
    ot() {
      ...
      for (const [s, n] of this.plugins.entries())
        n !== ht.copyRedirectedCacheableResponsesPlugin &&
          (n === ht.defaultPrecacheCacheabilityPlugin && (t = s), n.cacheWillUpdate && e++)
      0 === e
        ? this.plugins.push(ht.defaultPrecacheCacheabilityPlugin)
        : e > 1 && null !== t && this.plugins.splice(t, 1)
    }
  }
  ;((ht.defaultPrecacheCacheabilityPlugin = {
    cacheWillUpdate: async ({ response: t }) => (!t || t.status >= 400 ? null : t),
  }),
    (ht.copyRedirectedCacheableResponsesPlugin = {
      cacheWillUpdate: async ({ response: t }) => (t.redirected ? await ot(t) : t),
    }))
  class ut {
    ...
    addToCacheList(t) {
      ...
      for (const n of t) {
        ...
        const { cacheKey: t, url: i } = nt(n),
          r = 'string' != typeof n && n.revision ? 'reload' : 'default'
        if (this.ht.has(i) && this.ht.get(i) !== t)
          throw new s('add-to-cache-list-conflicting-entries', {
            firstEntry: this.ht.get(i),
            secondEntry: t,
          })
        if ('string' != typeof n && n.integrity) {
          if (this.lt.has(t) && this.lt.get(t) !== n.integrity)
            throw new s('add-to-cache-list-conflicting-integrities', { url: i })
          this.lt.set(t, n.integrity)
        }
        if ((this.ht.set(i, t), this.ut.set(i, r), e.length > 0)) {
    ...
    install(t) {
      return st(t, async () => {
        ...
        for (const [e, s] of this.ht) {
          const n = this.lt.get(s),
            i = this.ut.get(e),
  ...
  ;((t.BackgroundSyncPlugin = class {
    constructor(t, e) {
      ;((this.fetchDidFail = async ({ request: t }) => {
        await this.wt.pushRequest({ request: t })
      }),
        (this.wt = new et(t, e)))
    }
  }),
    ...
    (t.CacheableResponsePlugin = class {
      constructor(t) {
        ;((this.cacheWillUpdate = async ({ response: t }) =>
          this.dt.isResponseCacheable(t) ? t : null),
          (this.dt = new H(t)))
      }
    }),
    (t.ExpirationPlugin = class {
      ...
      gt(t) {
        ...
        let e = this.qt.get(t)
        return (e || ((e = new $(t, this.Rt)), this.qt.set(t, e)), e)
      }
      yt(t) {
        if (!this.T) return !0
        const e = this.bt(t)
      ...
      async deleteCacheAndMetadata() {
        for (const [t, e] of this.qt) (await self.caches.delete(t), await e.delete())
        this.qt = new Map()
      }
    }),
    (t.NetworkFirst = class extends q {
      constructor(t = {}) {
        ;(super(t),
          ...
          (this.vt = t.networkTimeoutSeconds || 0))
      }
      async _(t, e) {
        const n = [],
          i = []
        let r
        if (this.vt) {
          const { id: s, promise: a } = this.Dt({ request: t, logs: n, handler: e })
          ;((r = s), i.push(a))
        }
        const a = this.Et({ timeoutId: r, request: t, logs: n, handler: e })
        i.push(a)
